...
check out this comparison of the three types. when making a shape like this with all three layouts we can make some observations: the blob layout can produce convex and concave shapes without column mishaps, the edge layout does not have a solid interior tile so it creates columns at every corner, and the marching square layout does not extend to the edge of a tile. all three layouts do fine at both concave and convex shapes but the edge layout really does best for paths and lines instead of shapes. you would use it for a highway not a parking lot.